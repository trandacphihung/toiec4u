...
function getVolcabInfo(index) {
  ...
  $.getJSON("toiec4u/js/listvacab.json", function (data) {
    var vocab;
    var voice;
    var word_type;
    var phonetic;
    ...
    var titleDetail =
      '<h5 class="custom-title"><i class="fa fa-info-circle"></i>&nbsp&nbsp&nbspVocabulary Info</h5>';

    var contentDetailList = [];

    var indexToggleList = [];

    var indexAudioList = [];

    var indexAudioPlayList = [];
    ...
    for (let j = 0; j < a; j++) {
      contentDetail =
        ...
        listofvocabRender[j] +
        '"><img src="/image/audio_name_initial.svg" alt=""></a>';

      contentDetailList.push(contentDetail);

      indexToggle = '[data-toggle="popover-' + listofvocabRender[j] + '"]';

      indexToggleList.push(indexToggle);

      indexAudio = "popover-" + listofvocabRender[j];

      indexAudioList.push(indexAudio);

      indexAudioPlay = ".popover-body .aud-" + listofvocabRender[j];

      indexAudioPlayList.push(indexAudioPlay);
    }

    for (let i = 0; i < indexToggleList.length; i++) {
      $(document).ready(function () {
        $(indexToggleList[i]).popover({
          title: titleDetail,
          content: contentDetailList[i],
          html: true,
          placement: "bottom",
        ...
        $(document).on("click", indexAudioPlayList[i], function () {
          var audio = document.getElementById(indexAudioList[i]);
          audio.play();
        });
      });
    }
